...
"""
make_ani = True # If you want to create an animation for each recording put True
...
def plot_recordings(eq):
    ...
    plt.savefig(eq[0][:eq[0].index('/')] + '.png', dpi=300, facecolor=fig.get_facecolor()) # Save the figure as a .png
    plt.close()
...
def plot_filtered(dt, vals, filt_vals, filt_sum, cutoffs, file, title):
    ...
    ax.xaxis.set_tick_params(which='minor', bottom=False) # Minor ticks on the y-axis
    fig.patch.set_alpha(0)
    plt.savefig(file+' '+title+'.png', dpi=300, facecolor=fig.get_facecolor()) # Save the plot as a .png
    plt.close()
...
# Increase the font size on all figures and change text colour to sample from Inferno colormap
font = {'size':'15'}
plt.rc('font', **font)
plt.rcParams.update({'text.color':(0.085625, 0.044216, 0.221732), 'axes.labelcolor':(0.085625, 0.044216, 0.221732),
                        'xtick.labelcolor':(0.085625, 0.044216, 0.221732), 'ytick.labelcolor':(0.085625, 0.044216, 0.221732)})
